...
public class ListActivity extends quickmsg_activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    String email_address = preferences.get("email_address");
	...
	public void send_background(Integer extra)
	{
		Intent bg;
		
		bg = new Intent(this, background.class);
		bg.putExtra("state", extra);
		...
		for (int i = 0; i < contacts.size(); i++) {
			...
			if (contact.type_get() == contact.TYPE_PERSON) {
				...
				switch (contact.keystat_get()) {
					case net.vreeken.quickmsg.contact.KEYSTAT_NONE:
						Log.d("list", "keystat none");
						view_html += "color='#990000'";
						break;
					case net.vreeken.quickmsg.contact.KEYSTAT_RECEIVED:
						Log.d("list", "keystat received");
						view_html += "color='#cc6600'";
						break;
					case net.vreeken.quickmsg.contact.KEYSTAT_VERIFIED:
	...
	void warn_settings_dialog()
	{
    	...
    	for (int i = 0; i < account_types.account_types.size(); i++) {
    	    ...
    	    button.setText(account_types.account_types.get(i).name_get());
        ...
        alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
    		public void onClick(DialogInterface dialog, int whichButton) {
    			...
    			int idx = group.indexOfChild(button);
    			account = account_types.account_types.get(idx);

    			Log.d("selected", account.name_get());
	...
	void settings_set_account(account_type account, String name, String address, String pass)
	{
		account.preferences_set(getBaseContext(), name, address, pass);
	...
	void settings_done()
	{
	    ...
	    me.id_set(1);
	    me.name_set(preferences.get("display_name"));
	...
	private BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
		  ...
		  public void onReceive(Context context, Intent intent) {
		    ...
		    textview.setText(getString(R.string.status_connection) + ": " + 
		        getString(status ? R.string.status_connected : R.string.status_disconnected));
		    textview.setTextColor(status ? 
		    	Color.parseColor("#004400") : Color.parseColor("#990000"));
		  }
	};
