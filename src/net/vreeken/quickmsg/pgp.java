...
public class pgp {
	...
	{
		if (!pgp.load_keys() && my_user_id != null) {
	...
	public pgp(Context app_context)
	{
		...
		/*
		 * Older versions stored the key rings in the external files dir.
		 * Newer versions store it inside the sqlite db.
		 * Not much safer, but at least it is not on the SD by default.
		 * (if you can't keep your phone's filesystem safe you are f*ed any way....)
		 */
	    ...
	    String user_id = preferences.get("email_address", null);
	    if (user_id == null)
	    	return;
	...
	public Boolean load_keys()
	{
		...
		try {
			byte[] pubb = db.preference_get("pub");
			byte[] secb = db.preference_get("sec");

			if (pubb.length > 0 && secb.length > 0) {
				public_keyring_collection = new PGPPublicKeyRingCollection(pubb);
				secret_keyring = new PGPSecretKeyRing(secb);
				return true;
			}
		} 
		catch (Exception e) {
		...
		try {	
			...
			pubin.close();
			db.preference_add("pub", public_keyring_collection.getEncoded());
			secret_keyring = new PGPSecretKeyRing(secin);
			secin.close();
			db.preference_add("sec", secret_keyring.getEncoded());
		} 
		catch (Exception e) {
			Log.e("load_keys", e.getMessage());
			return false;
	...
	public void generate_keys(String email_address)
	{
		...
		try {
			...
			byte[] pubb = pubout.toByteArray();
			db.preference_add("pub", pubb);
	        pubout.close();
			...
			byte[] secb = secout.toByteArray();
			db.preference_add("sec", secb);
	        secout.close();
		}
		catch (Exception e){
